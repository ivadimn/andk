...
class MainRepository  {
    ...
    private val sharedPref : SharedPreferences
        = ru.ivadimn.MaterialDesign.App.context.getSharedPreferences(SHARED_PREFERENCES_NAME, Context.MODE_PRIVATE)
    ...
    private suspend fun saveImage(url : String) {
        ...
        withContext(Dispatchers.IO) {
            ...
            if (pendingUri != null) {
                download(
                    url,
                    ru.ivadimn.MaterialDesign.App.context.contentResolver.openOutputStream(pendingUri!!)
                )
                clearPending()
            }
        }
    ...
    private fun saveImageDetail(name: String, url: String): Uri? {
        ...
        val pUri = ru.ivadimn.MaterialDesign.App.context.contentResolver.insert(collectionUri, values)
        return pUri
    }
    ...
    private fun clearPending() {
        if (haveQ()) {
            pendingUri?.let {
                ru.ivadimn.MaterialDesign.App.context.contentResolver.update(it,
                    ContentValues().apply {
